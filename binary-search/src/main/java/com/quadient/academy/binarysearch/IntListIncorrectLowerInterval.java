...
public class IntListIncorrectLowerInterval {
    private List<Integer> list;

    public IntListIncorrectLowerInterval(List<Integer> list) {
        this.list = list;
    }
    ...
    private int runBinarySearchRecursively(List<Integer> sortedList, int key, int low, int high) {
        ...
        if (key == sortedList.get(middle)) {
            ...
            return runBinarySearchRecursively(
                    sortedList, key, low + 1, middle - 1);
        } else {
            return runBinarySearchRecursively(
                    sortedList, key, middle + 1, high);
        }
    }
}
